        ...
        match screen {
                                    ...
                                    match mods_clone.list_mods(&game_id_clone).await {
                                        Ok(updated_mods) => {
                                            ...
                                            st.set_status(format!(
                                                "✓ Rescan complete: {} mods imported, {} skipped",
                                                imported, skipped
                                            ));
                                        }
                                        Err(e) => {
                ...
                match key {
                    ...
                    KeyCode::Char('S') => {
                        ...
                        if let Some(_game) = &state.active_game {
                            ...
                            match plugins::loot::sort_plugins_native(&mut plugins_to_sort) {
                                Ok(_) => {
                                    // Validation
                                    let issues = plugins::sort::validate_load_order(&plugins_to_sort);
            ...
            Screen::Settings => {
                match key {
                    ...
                    KeyCode::Down | KeyCode::Char('j') => {
                        if state.selected_setting_index < 4 {
                            state.selected_setting_index += 1;
                        }
                    }
                    KeyCode::Enter => {
