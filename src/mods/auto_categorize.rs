...
const CATEGORY_RULES: &[CategoryRule] = &[
    ...
    CategoryRule {
        ...
        file_patterns: &[
            "meshes/architecture/whiterun",
            "meshes/architecture/solitude",
        ],
    },
    ...
    CategoryRule {
        category_name: "Individual Items",
        keywords: &[
            "clothing", "cloth", "outfit", "robe", "dress", "shirt", "pants", "skirt", "coat",
            "cloak", "cape", "hood", "cowl", "glove", "shoe", "sandal", "belt", "jewelry", "ring",
    ...
    CategoryRule {
        ...
        file_patterns: &[
            "textures/actors/character/female/eyes",
            "textures/actors/character/male/eyes",
        ],
    },
    // Appearance - General
    CategoryRule {
...
pub async fn auto_categorize_mod(db: &Database, mod_info: &InstalledMod) -> Result<()> {
    ...
    for rule in CATEGORY_RULES {
        ...
        for pattern in rule.file_patterns {
            if file_paths
                .iter()
                .any(|path: &String| path.contains(pattern))
            {
                score += 5;
            }
        }
    ...
    let (final_category, score) = if let Some((cat, score)) = best_match {
        ...
        let has_sounds = file_paths
            .iter()
            .any(|p: &String| p.contains("sound/") || p.contains("music/"));
        let has_interface = file_paths.iter().any(|p: &String| p.contains("interface/"));
        let has_esp = file_paths
            .iter()
            .any(|p: &String| p.ends_with(".esp") || p.ends_with(".esm") || p.ends_with(".esl"));
        ...
        if has_textures && !has_meshes {
            ...
            if file_paths.iter().any(|p: &String| p.contains("actors/")) {
                ...
                .iter()
                .any(|p: &String| p.contains("weapons/") || p.contains("armor/"))
            {
                ("Individual Items", 3)
            } else if file_paths
                .iter()
                .any(|p: &String| p.contains("architecture/") || p.contains("clutter/"))
            {
                ...
                .any(|p: &String| p.contains("weapons/") || p.contains("armor/"))
            {
                ...
                .iter()
                .any(|p: &String| p.contains("architecture/"))
            {
                ("Individual Buildings", 3)
            } else {
                ("Foliage Mods", 3)
