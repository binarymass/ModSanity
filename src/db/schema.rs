...
/// Indexed plugin-to-mod association for fast plugin-name resolution.
#[derive(Debug, Clone)]
pub struct ModPluginIndexRecord {
    pub id: Option<i64>,
    pub mod_id: i64,
    pub game_id: String,
    pub plugin_name: String,
    pub plugin_name_lower: String,
}

impl ModPluginIndexRecord {
    pub fn from_row(row: &Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            id: Some(row.get(0)?),
            mod_id: row.get(1)?,
            ...
            plugin_name_lower: row.get(4)?,
        })
    }
}

/// Lookup hit for plugin -> installed mod mapping.
#[derive(Debug, Clone)]
pub struct PluginModLookupHit {
    pub mod_id: i64,
    pub mod_name: String,
